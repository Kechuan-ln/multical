...
def export_json(calib, names, filenames, master=None):  
  ...
  base_in_world = calib.motion.pose_table
  base_tags=[i for i in range(base_in_world.valid.shape[0])]
  ...
  data = struct(
    ...
    camera_world2base = export_camera_poses(base_tags, base_in_world),
    camera_base2cam = export_camera_poses(names.camera, camera_poses)\
