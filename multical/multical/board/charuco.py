...
class CharucoBoard(Parameters, Board):
  ...
  def detect(self, image):    
    marker_corners, marker_ids, _ = cv2.aruco.detectMarkers(image,  self.board.dictionary, parameters=aruco_config(self.aruco_params)) 
    if marker_ids is None: return empty_detection

    _, corners, ids = cv2.aruco.interpolateCornersCharuco(marker_corners, marker_ids, image, self.board)
